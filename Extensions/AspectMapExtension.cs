...
internal static class AspectMapExtension
{
  ...
  private static readonly Dictionary<string, string> ExplicitAffixMappings = new()
  {
    ...
    //{"??","fire damage ranks of the inner flames passive"},
    //{"??","damage reduction from enemies that are affected by shadow damage over time"},
    //{"??","crowd control duration lucky hit up to a chance to heal life"},
  };

  public static string TryGetAspectMapping(this string aspectName) 
    => ExplicitAspectsMappings.GetValueOrDefault(aspectName, aspectName);

  public static string TryGetAffixMapping(this string maxrollAffixDescription)
